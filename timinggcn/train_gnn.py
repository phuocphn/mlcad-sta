...
parser.add_argument(
    "--test_iter", type=int, help="If specified, test a saved model instead of training"
)
parser.add_argument(
    "--checkpoint",
    type=str,
    ...
    with torch.no_grad():
        ...
        def test_dict(data):
            for k, (g, ts) in data.items():
                ...
                r2 = r2_score(
                    pred.cpu().numpy().reshape(-1), truth.cpu().numpy().reshape(-1)
                )
...
def train(model, args):
    ...
    for e in range(100000):
        model.train()
        train_loss_tot_net_delays, train_loss_tot_cell_delays, train_loss_tot_ats = (
            0,
            0,
            0,
        )
        ...
        for k, (g, ts) in random.sample(data_train.items(), batch_size):
            pred_net_delays, pred_cell_delays, pred_atslew = model(
                g, ts, groundtruth=args.groundtruth
            )
            loss_net_delays, loss_cell_delays = 0, 0

            if args.netdelay:
                loss_net_delays = F.mse_loss(
                    pred_net_delays, g.ndata["n_net_delays_log"]
                )
                train_loss_tot_net_delays += loss_net_delays.item()

            if args.celldelay:
                loss_cell_delays = F.mse_loss(
                    pred_cell_delays, g.edges["cell_out"].data["e_cell_delays"]
                )
        ...
        if e == 0 or e % 20 == 19:
            with torch.no_grad():
                ...
                for k, (g, ts) in data_test.items():
                    pred_net_delays, pred_cell_delays, pred_atslew = model(
                        g, ts, groundtruth=True
                    )
                    _, pred_cell_delays_prop, pred_atslew_prop = model(
                        g, ts, groundtruth=False
                    )

                    if args.netdelay:
                        test_loss_tot_net_delays += F.mse_loss(
                            pred_net_delays, g.ndata["n_net_delays_log"]
                        ).item()
                    if args.celldelay:
                        test_loss_tot_cell_delays += F.mse_loss(
                            pred_cell_delays, g.edges["cell_out"].data["e_cell_delays"]
                        ).item()
                    test_loss_tot_ats += F.mse_loss(
                        pred_atslew, g.ndata["n_atslew"]
                    ).item()
                    test_loss_tot_ats_prop += F.mse_loss(
                        pred_atslew_prop, g.ndata["n_atslew"]
                    ).item()

                print(
                    "Epoch {}, net delay {:.6f}/{:.6f}, cell delay {:.6f}/{:.6f}, at {:.6f}/({:.6f}, {:.6f})".format(
                        e,
            ...
            if (
                e == 0
                or e % 200 == 199
                or (e > 6000 and test_loss_tot_ats_prop / len(data_test) < 6)
            ):
                if args.checkpoint:
...
if __name__ == "__main__":
    ...
    if args.test_iter:
        ...
        model.load_state_dict(
            torch.load(
                "./checkpoints/{}/{}.pth".format(args.checkpoint, args.test_iter)
            )
        )
        test(model)
        test_netdelay(model)
        ...
        if args.checkpoint:
            ...
            print(
                "No checkpoint is specified. abandoning all model checkpoints and logs"
            )
            train(model, args)
